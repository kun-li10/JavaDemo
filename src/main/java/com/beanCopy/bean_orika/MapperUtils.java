...
/**
 * 使用枚举实现的单例模式创建一个映射工具类
 *
 * @author lk
 * @version 1.0
 * @date 2020/12/26 23:57
 */
public enum MapperUtils {
  ...
  public <E, T> List<E> mapAsList(Class<E> toClass, Collection<T> data,
    ...
    T t = data.stream().findFirst().orElseThrow(() -> new ExceptionInInitializerError(
        "映射集合，数据集合为空"));
   ...
   * @return 映射类对象
   */
  private <E, T> MapperFacade getMapperFacade(Class<E> toClass, Class<T> dataClass, Map<String,
      String> configMap) {
