...
/** 自定义IOC bean初始化管理 @Author lk @Date 2020/4/1 16:38 @Version 1.0 */
public class SelfPathXmlApplicationContext {
  ...
  void setBeans(Class<?> clz) throws Exception {
    ...
    if (present) {
      ...
      if (null != annotation) {
        String className;
        if (!StringUtils.isEmpty(annotation.value())) className = annotation.value();
        else className = clz.getSimpleName();
  ...
  public void attriAssign(Object bean) throws IllegalAccessException {
    ...
    for (Field field : fields) {
      ...
      if (flag) {
        ...
        if (!StringUtils.isEmpty(value)) beanID = value;
        else beanID = field.getName(); // bean的名称
  ...
  public static String toLowCaseFirstOne(String key) {
    if (Character.isLowerCase(key.charAt(0))) return key;
    else
      return (new StringBuilder())
          .append(Character.toLowerCase(key.charAt(0)))
